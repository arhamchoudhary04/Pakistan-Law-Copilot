...
<p align="center">
  <img src="docs/architecture.svg" alt="System architecture: client, API, agent, models and stores, and the offline ingestion lane" width="100%">
</p>
...
The diagrams here are generated by [`docs/diagrams.py`](docs/diagrams.py) in the app's own
palette, so they cannot drift from the UI by hand-editing.
...
<p align="center">
  <img src="docs/agent-flow.svg" alt="The agent as a state machine: rewrite, retrieve, grade, rerank, generate, verify, with a refusal branch and a retry loop" width="100%">
</p>

### Three layers between a question and a wrong answer

| | Layer | What it stops | Where it fails safe |
|:--:|---|---|---|
| **1** | **Cosine relevance gate** | Answering a question the corpus does not cover | Refuses **before any LLM call**, and reports the score it saw |
| **2** | **Structured citations** | A fabricated section number | Markers resolve to chunk **ids**; an invalid one cannot be rendered |
| **3** | **Self-verification** | A claim the sources do not support | Loops back to `rewrite` with feedback, up to `MAX_ATTEMPTS` |
...
%%{init: {'theme':'base','themeVariables':{
  'background':'#0E1117','primaryColor':'#1C222C',
  'primaryTextColor':'#E9EEF5','primaryBorderColor':'#A78BFA','lineColor':'#FBBF24',
  'secondaryColor':'#232B37','tertiaryColor':'#151A22','textColor':'#9AA6B8',
  'actorBkg':'#1C222C','actorBorder':'#22D3EE','actorTextColor':'#E9EEF5',
  'actorLineColor':'#3B475A','signalColor':'#FBBF24','signalTextColor':'#9AA6B8',
  'labelBoxBkgColor':'#232B37','labelBoxBorderColor':'#A78BFA','labelTextColor':'#E9EEF5',
  'loopTextColor':'#9AA6B8','altSectionBkgColor':'#151A22',
  'noteBkgColor':'#232B37','noteTextColor':'#E9EEF5','noteBorderColor':'#34D399',
  'sequenceNumberColor':'#0E1117'}}}%%
sequenceDiagram
    autonumber
...
![React](https://img.shields.io/badge/React-18.3-61DAFB?logo=react&logoColor=20232A)
![TypeScript](https://img.shields.io/badge/TypeScript-5.6-3178C6?logo=typescript&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-3.4-06B6D4?logo=tailwindcss&logoColor=white)
![PostCSS](https://img.shields.io/badge/PostCSS-8.4-DD3A0A?logo=postcss&logoColor=white)
![SSE](https://img.shields.io/badge/SSE-hand--written_client-7C3AED)

**Backend**

![Python](https://img.shields.io/badge/Python-3.11+-3776AB?logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-0.115-009688?logo=fastapi&logoColor=white)
![Pydantic](https://img.shields.io/badge/Pydantic-v2.9-E92063?logo=pydantic&logoColor=white)
![Uvicorn](https://img.shields.io/badge/Uvicorn-0.32-499848)
![Starlette](https://img.shields.io/badge/sse--starlette-2.1-1E5B94)
![httpx](https://img.shields.io/badge/httpx-0.27-2A6DB2)

**AI and retrieval**

![LangGraph](https://img.shields.io/badge/LangGraph-0.2-1C3C3C?logo=langchain&logoColor=white)
![Groq](https://img.shields.io/badge/Groq-llama--3.1--8b-F55036)
![ONNX](https://img.shields.io/badge/ONNX_Runtime-fastembed-005CED?logo=onnx&logoColor=white)
![bge-small](https://img.shields.io/badge/bge--small--en--v1.5-384--dim-FFB000)
![cross-encoder](https://img.shields.io/badge/cross--encoder-ms--marco--MiniLM-8B5CF6)
![NumPy](https://img.shields.io/badge/NumPy-1.26-013243?logo=numpy&logoColor=white)

**Data stores**

![FAISS](https://img.shields.io/badge/FAISS-IndexFlatIP-0467DF?logo=meta&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-accounts_+_history-003B57?logo=sqlite&logoColor=white)
![Neo4j](https://img.shields.io/badge/Neo4j-5.24_provision_graph-4581C3?logo=neo4j&logoColor=white)
![Markdown](https://img.shields.io/badge/Markdown_corpus-16_acts-6E7A8C?logo=markdown&logoColor=white)

**Ingestion and auth**

![pypdf](https://img.shields.io/badge/pypdf-5.0-B7472A)
![PBKDF2](https://img.shields.io/badge/PBKDF2--HMAC--SHA256-200k_iterations-2E7D32)
![HMAC](https://img.shields.io/badge/HMAC-typed_session_tokens-2E7D32)
![Rate limit](https://img.shields.io/badge/Rate_limiting-stdlib_sliding_window-D97706)

**Quality and CI**

![pytest](https://img.shields.io/badge/pytest-125_passing-0A9EDC?logo=pytest&logoColor=white)
![ruff](https://img.shields.io/badge/ruff-clean-D7FF64?logo=ruff&logoColor=black)
![mypy](https://img.shields.io/badge/mypy-36_files_clean-2A6DB2)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-6_gates-2088FF?logo=githubactions&logoColor=white)
![Eval](https://img.shields.io/badge/Golden_set-50_questions-16A34A)

### External services and credentials

Exactly one API key is needed, and only for generation. Everything else runs locally.

| Service | Credential | Required? | What breaks without it |
|---|---|---|---|
| **Groq** (`llama-3.1-8b-instant`) | `GROQ_API_KEY` | For generated answers only | Retrieval, the relevance gate, the refusal path, and the whole eval suite still work; answers come back as `partial` |
| **Neo4j** (optional graph) | `NEO4J_URI`, `NEO4J_USERNAME`, `NEO4J_PASSWORD` | No | Retrieval silently falls back to vector-only |
| Embeddings and reranker | none | n/a | Local ONNX, downloaded once from Hugging Face at build time |
| SQLite | none | n/a | A file under `DATA_DIR` |

`AUTH_SECRET` is not a third-party key: it is the local HMAC signing secret, and the app
refuses to boot in production while it is still the built-in default.
...
```mermaid
%%{init: {'theme':'base','themeVariables':{
  'background':'#0E1117','primaryColor':'#1C222C','primaryTextColor':'#E9EEF5',
  'primaryBorderColor':'#3B475A','lineColor':'#60A5FA','textColor':'#E9EEF5',
  'secondaryColor':'#232B37','tertiaryColor':'#151A22','fontSize':'14px'}}}%%
flowchart LR
    ...
    style P fill:#1C222C,stroke:#22D3EE,stroke-width:2px,color:#E9EEF5
    style L fill:#1C222C,stroke:#A3E635,stroke-width:2px,color:#E9EEF5
    style T fill:#1C222C,stroke:#A3E635,stroke-width:2px,color:#E9EEF5
    style U fill:#1C222C,stroke:#A3E635,stroke-width:2px,color:#E9EEF5
    style IX fill:#1C222C,stroke:#60A5FA,stroke-width:2px,color:#E9EEF5
    style EV fill:#232B37,stroke:#FBBF24,stroke-width:3px,color:#FCD34D
    style W fill:#1C222C,stroke:#A78BFA,stroke-width:2px,color:#E9EEF5
    style OK fill:#1C222C,stroke:#4ADE80,stroke-width:2px,color:#4ADE80
    style FAIL fill:#1C222C,stroke:#FB7185,stroke-width:2px,color:#FB7185
```
